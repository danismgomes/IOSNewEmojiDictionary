...
struct Emoji : Codable {
    ...
    var usage: String
    static let documentsDiretory = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first!
    
    static let archiveURL = documentsDiretory.appendingPathComponent("emojis").appendingPathExtension("plist")
    
    static func saveToFile(emojis: [Emoji]) {
        let propertyListEncoder = PropertyListEncoder()
        if let encodeEmoji = try? propertyListEncoder.encode(emojis) {
            do {
    ...
    static func loadFromFile() -> [Emoji] {
        ...
        if let retrievedEmojiData = try? Data(contentsOf: archiveURL) {
            if let decodeEmojis = try? propertyListDecoder.decode([Emoji].self, from: retrievedEmojiData) {
                return decodeEmojis
            }
        }
        return []
